...
files2d=`find ./docs/rustdoc/salva2d ./docs/rustdoc/src/salva2d -name \*.html -printf '%p '`
files3d=`find ./docs/rustdoc/salva3d ./docs/rustdoc/src/salva3d -name \*.html -printf '%p '`
...
do
  patch_file $file "rustdoc_salva2d"
done


for file in `echo $files3d`
do
  patch_file $file "rustdoc_salva3d"
done
